class Admin::ProductStocksController < AdminController
  ...
  def index
    @admin_product_stocks = ProductStock.where(product_id: params[:product_id])
  ...
  def new
    ...
    @admin_product_stock = ProductStock.new
  end
  ...
  def create
    @product = Product.find(params[:product_id])
    @admin_product_stock = @product.product_stocks.new(admin_product_stock_params)


    respond_to do |format|
      if @admin_product_stock.save
        format.html { redirect_to admin_product_product_stock_url(@product, @admin_product_stock), notice: "Product stock was successfully created." }
  ...
  private
    ...
    def set_admin_product_stock
      @admin_product_stock = ProductStock.find_by(params[:id])
    end

    # Only allow a list of trusted parameters through.
    def admin_product_stock_params
      params.require(:product_stock).permit(:size, :amount, :price)
    end
end
